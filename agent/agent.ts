...
async function postWithX402(
  client: ReturnType<typeof createCargoProofX402Client>,
  path: string,
  body: unknown
) {
  const url = `${BASE}${path}`;
  ...
  if (!response.ok) {
    throw new Error(
      `[x402] Paid request failed (${response.status}): ${JSON.stringify(
        data
      )}`
    );
  }
...
async function main() {
  ...
  if (!resetResponse.ok) {
    throw new Error(
      `Failed to reset CargoProof: ${resetResponse.status}`
    );
  }
  ...
  const aisArrival =
    ...
    (
      ("arrival" in ais &&
        Boolean(
          (ais as { arrival?: unknown }).arrival
        )) ||
  ...
  if (!aisArrival) {
    console.log("");
    console.log(
      "[AGENT] AIS does not support arrival."
    );

    console.log(
      "[AGENT] Purchasing independent port evidence..."
    );

    const port = await postWithX402(
      client,
  ...
  console.log(
    "[AGENT] x402 evidence procurement complete"
  );
