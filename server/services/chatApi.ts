...
// ------------------------------------------------------------
// The dashboard/demo refers to shipments as CP001 (clean) / CP002 (fraud).
// The underlying evidence store keys them as CP-CLEAN / CP-FRAUD. We accept
// either form from the user and map to both a display ID and a scenario.

type ShipmentAlias = {
  displayId: string;
  storeId: string;
  scenario: "clean" | "fraud";
};

const SHIPMENT_ALIASES: ShipmentAlias[] = [
  { displayId: "CP001", storeId: "CP-CLEAN", scenario: "clean" },
  { displayId: "CP002", storeId: "CP-FRAUD", scenario: "fraud" },
];

function resolveShipmentAlias(raw: string): ShipmentAlias | null {
  const normalized = raw.trim().toUpperCase().replace(/\s+/g, "");
  return (
    SHIPMENT_ALIASES.find(
      (a) =>
        a.displayId === normalized ||
        a.storeId === normalized.replace(/^CP0*/, "CP-").replace("CP-", "CP-") ||
        normalized === a.storeId ||
        normalized.replace("-", "") === a.storeId.replace("-", "")
    ) || null
  );
}
 ...
 * generic help. */
function extractRawShipmentToken(message: string): string | null {
  const match = message.toUpperCase().match(/\bCP[-\s]?[A-Z0-9]{1,10}\b/);
  return match ? match[0] : null;
}

function extractShipmentId(message: string): ShipmentAlias | null {
...
function hasUnknownShipmentReference(message: string): boolean {
  const token = extractRawShipmentToken(message);
  return Boolean(token) && !resolveShipmentAlias(token!);
}

function mentionsTransaction(message: string): boolean {
  return /transaction|\btx\b/i.test(message);
}

function extractTransactionId(message: string): string | null {
...
function detectIntent(
  ...
  if (txId) return "TRANSACTION_LOOKUP";
  ...
  if (mentionsTransaction(message)) return "TRANSACTION_LOOKUP";

  if (!shipment) {
    if (hasUnknownShipmentReference(message)) return "SHIPMENT_LOOKUP";
  ...
  if (/evidence/.test(m)) return "EVIDENCE_LOOKUP";

  if (/escrow/.test(m)) return "ESCROW_LOOKUP";
...
async function getShipmentBundle(alias: ShipmentAlias) {
...
async function runVerification(alias: ShipmentAlias) {
 ...
 */
async function ensureStateFor(alias: ShipmentAlias) {
...
function extractLatestTxFromEvidence(evidence: any[]): string | null {
...
async function handleTransactionLookup(message: string, txId: string | null) {
  if (!txId) {
    return {
      message:
        "That doesn't look like a valid Algorand Testnet transaction ID. Transaction IDs are 52-character values using only letters A-Z and digits 2-7. Please double-check the ID and try again.",
      intent: "TRANSACTION_LOOKUP" as Intent,
      transaction: null,
      lora_url: null,
  ...
  try {
    ...
    return {
      message:
        "That doesn't look like a valid Algorand Testnet transaction ID. Transaction IDs are 52-character values using letters A-Z and digits 2-7.",
      intent: "TRANSACTION_LOOKUP" as Intent,
      transaction: null,
      lora_url: null,
      error: "INVALID_TRANSACTION_ID",
...
async function handleShipmentLookup(message: string, alias: ShipmentAlias | null) {
  if (!alias) {
    const token = extractRawShipmentToken(message);
    return {
      message: `I couldn't find shipment ${token || "that ID"} in CargoProof's data. Known shipments are CP001 and CP002.`,
      intent: "SHIPMENT_LOOKUP" as Intent,
      shipment_id: token,
      shipment: null,
    };
  }
...
async function handleVerify(message: string, alias: ShipmentAlias) {
  const result = await runVerification(alias);

  if (!result) {
    return {
      message:
        "I couldn't reach the CargoProof backend to run verification. Make sure the CargoProof server is running on port " +
        PORT +
        ".",
      intent: "VERIFY_SHIPMENT" as Intent,
      shipment_id: alias.displayId,
      verification: null,
    };
  }
  ...
  const fallback = [
    ...
    `Evidence purchased: ${(result.evidence || [])
      .map((e: any) => `${e.provider} ($${e.priceUsd})`)
      .join(", ")}`,
...
async function handleExplanation(message: string, alias: ShipmentAlias) {
  const state = await ensureStateFor(alias);

  if (!state) {
    return {
      message: "I couldn't reach the CargoProof backend to explain this shipment.",
      intent: "EXPLANATION" as Intent,
      shipment_id: alias.displayId,
    };
  }

  const decisionEvent = (state.events || []).find(
    (e: any) => e.type === "DECISION" && e.failedRules
  );
...
async function handleEvidenceLookup(message: string, alias: ShipmentAlias) {
  const state = await ensureStateFor(alias);
  if (!state) {
    return { message: "Couldn't reach the CargoProof backend.", intent: "EVIDENCE_LOOKUP" as Intent, shipment_id: alias.displayId };
  }

  const structured = { shipment_id: alias.displayId, evidence: state.evidence };
  const fallback = (state.evidence || [])
    .map((e: any) => `${e.provider}: ${e.observed} (paid $${e.priceUsd}, ${e.simulated ? "simulated source" : "live source"})`)
    .join("\n") || "No evidence has been collected yet for this shipment.";

  const llm = await explainWithLLM(message, structured, fallback);
  return { message: llm.text, intent: "EVIDENCE_LOOKUP" as Intent, shipment_id: alias.displayId, evidence: state.evidence };
}

async function handlePaymentLookup(message: string, alias: ShipmentAlias) {
  const state = await ensureStateFor(alias);
  if (!state) {
    return { message: "Couldn't reach the CargoProof backend.", intent: "PAYMENT_LOOKUP" as Intent, shipment_id: alias.displayId };
  }
  ...
  const structured = {
    ...
    payments: (state.evidence || []).map((e: any) => ({ provider: e.provider, price_usd: e.priceUsd, tx_id: e.txId })),
  ...
  const fallback = `Total evidence cost for ${alias.displayId}: $${state.spendUsd?.toFixed?.(2) ?? state.spendUsd}. ${(state.evidence || [])
    .map((e: any) => `${e.provider} cost $${e.priceUsd}`)
    .join("; ")}`;

  const llm = await explainWithLLM(message, structured, fallback);
  return {
    message: llm.text,
    intent: "PAYMENT_LOOKUP" as Intent,
    shipment_id: alias.displayId,
...
async function handleEscrowLookup(message: string, alias: ShipmentAlias) {
  const state = await ensureStateFor(alias);
  if (!state) {
    return { message: "Couldn't reach the CargoProof backend.", intent: "ESCROW_LOOKUP" as Intent, shipment_id: alias.displayId };
  }
  ...
  const llm = await explainWithLLM(message, structured, fallback);
  return { message: llm.text, intent: "ESCROW_LOOKUP" as Intent, shipment_id: alias.displayId, escrow: { status: state.escrow } };
}

async function handleGeneralHelp(message: string) {
  const structured = {
    capabilities: [
      "Verify CP001 / CP002",
  ...
  const fallback =
    "I'm the CargoProof Assistant. Ask me things like \"Verify CP001\", \"Why is CP002 suspicious?\", \"Show evidence for CP001\", \"What is the escrow status?\", or paste an Algorand transaction ID to have it explained.";

  const llm = await explainWithLLM(message, structured, fallback);
  return { message: llm.text, intent: "GENERAL_CARGOPROOF_HELP" as Intent };
}
...
chatApi.post("/api/v1/chat", async (c) => {
  ...
  if (!message) {
    return c.json(
      { message: "Please send a non-empty message (max 2000 characters).", intent: "GENERAL_CARGOPROOF_HELP" },
      400
    );
  }
