...
const INDEXER_URL =
  process.env.INDEXER_URL || "https://testnet-idx.algonode.cloud";
...
export async function getAccountInfo(address: string) {
  const response = await fetch(
    `${ALGOD_URL}/v2/accounts/${encodeURIComponent(address)}`
  );
 ...
 */
export function isValidAlgorandTxId(value: string): boolean {
  return /^[A-Z2-7]{52}$/.test((value || "").trim().toUpperCase());
}

export function buildLoraTestnetTxUrl(txId: string): string {
  return `https://lora.algokit.io/testnet/transaction/${txId}`;
}

export function buildLoraTestnetAddressUrl(address: string): string {
 ...
 */
export async function getTransaction(rawTxId: string): Promise<any | null> {
  ...
  try {
    const response = await fetch(
      `${INDEXER_URL}/v2/transactions/${encodeURIComponent(txId)}`
    );
  ...
  try {
    const response = await fetch(
      `${ALGOD_URL}/v2/transactions/pending/${encodeURIComponent(txId)}`
    );
 ...
 */
export function summarizeTransaction(raw: any, txId: string) {
