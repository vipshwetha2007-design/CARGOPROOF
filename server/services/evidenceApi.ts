...
type Store = {
  ...
  container_events: Record<string, any[]>;
...
function offsetToIso(hoursAgo: number): string {
  const d = new Date(Date.now() - hoursAgo * 60 * 60 * 1000);
  return d.toISOString();
}

function resolveTimestamps(obj: any, field: string, offsetField: string) {
...
evidenceApi.get("/api/v1/shipments/:id", (c) => {
  ...
  if (!shipment) return c.json({ detail: "not found" }, 404);
...
evidenceApi.get("/api/v1/vessels/:imo", (c) => {
  ...
  if (!vessel) return c.json({ detail: "not found" }, 404);
...
evidenceApi.get("/api/v1/containers/:number", (c) => {
  ...
  if (!container) return c.json({ detail: "not found" }, 404);
...
evidenceApi.get("/api/v1/shipments/:id/inspections", (c) => {
  ...
  if (!inspection) return c.json(null, 404);
